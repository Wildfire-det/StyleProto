...
from detectron2.data.style_hallucination import add_style_hallucination_config, AugmentedDatasetMapper
#from detectron2.modeling.meta_arch.devit import DiscriminativePrototypes
...
class Trainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        dispro_params = [
            ...
            if "dispro" in n.lower()  
        ]
        
      
        other_params = [
            p for n, p in model.named_parameters() 
            if "dispro" not in n.lower()
        ]
        
    
        param_groups = [
            {"params": dispro_params, "lr": cfg.SOLVER.BASE_LR },  # 10%基础学习率
...
def setup(args):
    ...
    """
    cfg = get_cfg()
    add_style_hallucination_config(cfg)
    cfg.merge_from_file(args.config_file)
    cfg.merge_from_list(args.opts)
    cfg.DE.CONTROLLER = args.controller
    print(f"Mapper used: {cfg.DATALOADER.MAPPER}")
    cfg.freeze()
